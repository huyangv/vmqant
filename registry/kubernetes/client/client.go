...
func NewService(name, version, typ string) *Service {
	log.Debug("kubernetes default service: name: %s, version: %s", name, version)
...
func NewDeployment(name, version, typ string) *Deployment {
	log.Debug("kubernetes default deployment: name: %s, version: %s", name, version)
...
func NewClusterClient() *client {
	...
	if s == nil || !s.IsDir() {
		log.Error(errors.New("service account not found").Error())
	}
